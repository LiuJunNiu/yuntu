 ...
 */
void *IOT_OTA_Init(const char *product_id, const char *device_name,
                   void *ch_signal);
 ...
 * @return
 ...
 OTA_VERSION_LEN_MAX 5) if type==IOT_OTAG_CHECK_FIRMWARE, 'buf' = uint32_t
 pointer, 'buf_len' = 4
 *
 * @retval   0 : success
 * @retval < 0 : error code for failure
 */
int IOT_OTA_Ioctl(void *handle, IOT_OTA_CmdType type, void *buf,
                  size_t buf_len);
