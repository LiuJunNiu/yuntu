...
typedef enum {
    QCLOUD_RET_MQTT_ALREADY_CONNECTED =
        4,  // Already connected with MQTT server
    QCLOUD_RET_MQTT_CONNACK_CONNECTION_ACCEPTED =
        3,  // MQTT connection accepted by server
    QCLOUD_RET_MQTT_MANUALLY_DISCONNECTED =
        2,  // Manually disconnected with MQTT server
    QCLOUD_RET_MQTT_RECONNECTED =
        1,  // Reconnected with MQTT server successfully
    ...
    QCLOUD_ERR_MQTT_PUSH_TO_LIST_FAILED =
        -102,                        // Fail to push node to MQTT waiting list
    ...
    QCLOUD_ERR_MQTT_CONNACK_IDENTIFIER_REJECTED =
        -114,  // MQTT connection refused: identifier rejected
    QCLOUD_ERR_MQTT_CONNACK_SERVER_UNAVAILABLE =
        -115,  // MQTT connection refused: service not available
    QCLOUD_ERR_MQTT_CONNACK_BAD_USERDATA =
        -116,  // MQTT connection refused: bad user name or password
    QCLOUD_ERR_MQTT_CONNACK_NOT_AUTHORIZED =
        -117,  // MQTT connection refused: not authorized
    ...
    QCLOUD_ERR_SHADOW_UPDATE_REJECTED =
        -204,                               // Shadow update rejected by server
    QCLOUD_ERR_SHADOW_GET_TIMEOUT  = -205,  // Shadow get timeout
    QCLOUD_ERR_SHADOW_GET_REJECTED = -206,  // Shadow get rejected by server
    QCLOUD_ERR_GATEWAY_CREATE_SESSION_FAIL =
        -221,  // Gateway fail to create sub-device session
    QCLOUD_ERR_GATEWAY_SESSION_NO_EXIST =
        -222,  // Gateway sub-device session not exist
    QCLOUD_ERR_GATEWAY_SESSION_TIMEOUT =
        -223,  // Gateway sub-device session timeout
