...
typedef struct {
    ...
    uint32_t
            keep_alive_interval_ms;  // MQTT keep alive time interval in millisecond
 ...
 *
 */
typedef void (*OnRequestCallback)(void *pClient, Method method,
                                  RequestAck  requestAck,
                                  const char *pJsonDocument, void *userContext);
 ...
 */
typedef void (*OnPropRegCallback)(void *pClient, const char *pJsonValueBuffer,
 ...
 * @return packet id (>=0) when success, or err code (<0) for failure
 */
int IOT_Shadow_Subscribe(void *pClient, char *topicFilter,
 ...
 * failure
 */
int IOT_Shadow_Update(void *pClient, char *pJsonDoc, size_t sizeOfBuffer,
                      OnRequestCallback callback, void *userContext,
                      uint32_t timeout_ms);
 ...
 */
int IOT_Shadow_Update_Sync(void *pClient, char *pJsonDoc, size_t sizeOfBuffer,
                           uint32_t timeout_ms);
 ...
 * failure
 */
int IOT_Shadow_Get(void *pClient, OnRequestCallback callback, void *userContext,
                   uint32_t timeout_ms);
 ...
 * failure
 */
int IOT_Shadow_Register_Property(void *pClient, DeviceProperty *pProperty,
 ...
 * @return              QCLOUD_RET_SUCCESS when success, or err code for failure
 */
int IOT_Shadow_JSON_ConstructReport(void *pClient, char *jsonBuffer,
                                    size_t sizeOfBuffer, uint8_t count, ...);
...
int IOT_Shadow_JSON_ConstructReportArray(void *pClient, char *jsonBuffer,
                                         size_t sizeOfBuffer, uint8_t count,
 ...
 * @return              QCLOUD_RET_SUCCESS when success, or err code for failure
 */
int IOT_Shadow_JSON_Construct_OverwriteReport(void *pClient, char *jsonBuffer,
                                              size_t  sizeOfBuffer,
                                              uint8_t count, ...);

/**
 * @brief Add reported fields array in JSON document and set desire field as
 * null
 *
 * @param pClient       handle to shadow client
 * @param jsonBuffer    string buffer to store JSON document
 ...
 * @return              QCLOUD_RET_SUCCESS when success, or err code for failure
 */
int IOT_Shadow_JSON_ConstructReportAndDesireAllNull(void *  pClient,
                                                    char *  jsonBuffer,
                                                    size_t  sizeOfBuffer,
                                                    uint8_t count, ...);
 ...
 * @return              QCLOUD_RET_SUCCESS when success, or err code for failure
 */
int IOT_Shadow_JSON_ConstructDesireAllNull(void *handle, char *jsonBuffer,
                                           size_t sizeOfBuffer);
