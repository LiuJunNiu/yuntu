 ...
 */
typedef struct {
    bool need_resp;           // Need response from peer (true), or not (false).
    ...
    OnRespCallback resp_callback;  // function callback when recv msg response,
                                   // valid only when need_resp is true
} SendMsgParams;
...
typedef struct {
    ...
    uint32_t
                     command_timeout;  // coap timeout value when waiting for ACK/response
 ...
 */
int IOT_COAP_SendMessage(void *pClient, char *topicName,
                         SendMsgParams *sendParams);
