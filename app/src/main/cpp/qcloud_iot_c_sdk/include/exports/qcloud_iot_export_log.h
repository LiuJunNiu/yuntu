 ...
 * @param level
 */
void IOT_Log_Gen(const char *file, const char *func, const int line,
                 const int level, const char *fmt, ...);

/* Simple APIs for log generation in different level */
#define Log_d(fmt, ...)                                            \
    IOT_Log_Gen(__FILE__, __FUNCTION__, __LINE__, eLOG_DEBUG, fmt, \
                ##__VA_ARGS__)
#define Log_i(fmt, ...) \
    IOT_Log_Gen(__FILE__, __FUNCTION__, __LINE__, eLOG_INFO, fmt, ##__VA_ARGS__)
#define Log_w(fmt, ...) \
    IOT_Log_Gen(__FILE__, __FUNCTION__, __LINE__, eLOG_WARN, fmt, ##__VA_ARGS__)
#define Log_e(fmt, ...)                                            \
    IOT_Log_Gen(__FILE__, __FUNCTION__, __LINE__, eLOG_ERROR, fmt, \
                ##__VA_ARGS__)
...
#define UPLOAD_DBG(fmt, ...)                                           \
    HAL_Printf(">>LOG-DBG>>%s(%d): " fmt "\n", __FUNCTION__, __LINE__, \
               ##__VA_ARGS__)
#else
#define UPLOAD_DBG(...)
#endif
#define UPLOAD_ERR(fmt, ...)                                           \
    HAL_Printf(">>LOG-ERR>>%s(%d): " fmt "\n", __FUNCTION__, __LINE__, \
               ##__VA_ARGS__)
