...
#define REMOTE_CONFIG_ERRCODE_DISABLE \
    1001  // Cloud platform has been disabled remote config
 ...
 * @param config_reply_errcode Config ErrCode; ERRCODE_REMOTE_CONFIG_SUCCESS can
 proc config_json other config_json is invalid
 * @param config_json          config json data string, format is {xxxx}; user
 config data at cloud platform, pointer is struct ConfigSubscirbeUserData
 json_buffer member
 * @param config_json_len      config json data length,
 * @return                     void
 */
typedef void (*OnConfigProcHandler)(void *client, int config_reply_errcode,
                                    char *config_json, int config_json_len);

typedef struct {
    OnConfigProcHandler
        on_config_proc;   // config proc callback when message arrived
    int json_buffer_len;  // sys json data need 60 B + config json data max len
    char *json_buffer;    // save json buffer [sys json data + config json data]
                        // ,sdk proc recv buff save config json data as
                        // OnConfigProcHandler functions config_json parameter
} ConfigSubscirbeUserData;

/**
 * @brief subscribe config topic
...
int IOT_Subscribe_Config(void *                   client,
                         ConfigSubscirbeUserData *config_sub_userdata,
                         int                      subscribe_timeout);

/**
 * @brief get config from MQTT server
 ...
 *                          otherwise, failure
 */
int IOT_Get_Config(void *client, char *json_buffer, int buffer_size,
                   int reply_timeout);
