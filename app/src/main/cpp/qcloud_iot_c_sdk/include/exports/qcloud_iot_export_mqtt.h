 ...
 */
typedef void (*OnMessageHandler)(void *pClient, MQTTMessage *message,
                                 void *pUserData);

/**
 * @brief Define MQTT SUBSCRIBE callback when event happened
 */
typedef void (*OnSubEventHandler)(void *pClient, MQTTEventType event_type,
                                  void *pUserData);
 ...
 * @return none
 */
typedef void (*MQTTEventHandleFun)(void *pclient, void *context,
                                   MQTTEventMsg *msg);
 ...
 * @return packet id (>=0) when success, or err code (<0) for failure
 */
int IOT_MQTT_Subscribe(void *pClient, char *topicFilter,
