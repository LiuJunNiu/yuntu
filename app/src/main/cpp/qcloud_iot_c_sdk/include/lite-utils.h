...
#define LITE_isdigit(c) \
    (((c) <= '9' && (c) >= '0') ? (LITE_TRUE) : (LITE_FALSE))
...
void  LITE_hexbuf_convert(unsigned char *digest, char *out, int buflen,
                          int uppercase);
...
#define foreach_json_keys_in(src, iter_key, keylist, pos)                       \
    for (keylist = (void *)LITE_json_keys_of((char *)src, ""),                  \
        pos      = (void *)list_first_entry((list_head_t *)keylist, json_key_t, \
                                       list),                              \
