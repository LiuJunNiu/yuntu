...
#define MAX_SIZE_OF_CLOUD_TOPIC \
    ((MAX_SIZE_OF_DEVICE_NAME) + (MAX_SIZE_OF_PRODUCT_ID) + 64 + 6)
...
typedef struct SubTopicHandle {
    ...
    OnMessageHandler
        message_handler;  // callback when msg of this subscription arrives
    OnSubEventHandler
          sub_event_handler;  // callback when event of this subscription happens
...
typedef struct Client {
    ...
    uint8_t is_ping_outstanding;  // 1 = ping request is sent while ping
    ...
    SubTopicHandle
        sub_handles[MAX_MESSAGE_HANDLERS];  // subscription handle array
 ...
 * @return QCLOUD_RET_SUCCESS for success, or err code for failure
 */
int qcloud_iot_mqtt_connect(Qcloud_IoT_Client *pClient,
                            MQTTConnectParams *pParams);
 ...
 */
int qcloud_iot_mqtt_publish(Qcloud_IoT_Client *pClient, char *topicName,
                            PublishParams *pParams);
 ...
 * @return packet id (>=0) when success, or err code (<0) for failure
 */
int qcloud_iot_mqtt_subscribe(Qcloud_IoT_Client *pClient, char *topicFilter,
                              SubscribeParams *pParams);
 ...
 */
bool qcloud_iot_mqtt_is_sub_ready(Qcloud_IoT_Client *pClient,
                                  char *             topicFilter);
...
int qcloud_iot_mqtt_yield_mt(Qcloud_IoT_Client *mqtt_client,
                             uint32_t           timeout_ms);
 ...
 * @return QCLOUD_RET_SUCCESS for success, or err code for failure
 */
int cycle_for_read(Qcloud_IoT_Client *pClient, Timer *timer,
                   uint8_t *packet_type, QoS qos);
...
int push_sub_info_to(Qcloud_IoT_Client *c, int len, unsigned short msgId,
                     MessageTypes type, SubTopicHandle *handler,
                     ListNode **node);

int serialize_pub_ack_packet(unsigned char *buf, size_t buf_len,
                             MessageTypes packet_type, uint8_t dup,
                             uint16_t packet_id, uint32_t *serialized_len);

int serialize_packet_with_zero_payload(unsigned char *buf, size_t buf_len,
                                       MessageTypes packetType,
                                       uint32_t *   serialized_len);

int deserialize_publish_packet(unsigned char *dup, QoS *qos, uint8_t *retained,
                               uint16_t *packet_id, char **topicName,
                               uint16_t *topicNameLen, unsigned char **payload,
                               size_t *payload_len, unsigned char *buf,
                               size_t buf_len);

int deserialize_suback_packet(uint16_t *packet_id, uint32_t max_count,
                              uint32_t *count, QoS *grantedQoSs,
                              unsigned char *buf, size_t buf_len);

int deserialize_unsuback_packet(uint16_t *packet_id, unsigned char *buf,
                                size_t buf_len);

int deserialize_ack_packet(uint8_t *packet_type, uint8_t *dup,
                           uint16_t *packet_id, unsigned char *buf,
                           size_t buf_len);
...
int mqtt_read_packet_rem_len_form_buf(unsigned char *buf, uint32_t *value,
