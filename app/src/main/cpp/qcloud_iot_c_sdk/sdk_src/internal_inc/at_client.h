...
typedef struct _at_client_ {
    ...
    void *lock;  // pre cmd take the lock wait for resp , another cmd need wait
                 // for unlock
...
#define at_client_recv(buf, size, timeout) \
    at_client_obj_recv(buf, size, timeout)

/* AT response object create and delete */
at_response_t at_create_resp(uint32_t buf_size, uint32_t line_num,
                             uint32_t timeout);
...
int         at_resp_parse_line_args(at_response_t resp, uint32_t resp_line,
                                    const char *resp_expr, ...);
int at_resp_parse_line_args_by_kw(at_response_t resp, const char *keyword,
                                  const char *resp_expr, ...);
