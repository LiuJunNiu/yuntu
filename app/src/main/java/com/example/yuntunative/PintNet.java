...
public class PintNet {
    ...
    public static PingNetEntity ping(PingNetEntity pingNetEntity) {
        ...
        String command =
                "ping -c " + pingNetEntity.getPingCount() + " -w " + pingNetEntity.getPingWtime() + " " + pingNetEntity.getIp();
//        String command = "ping -c " + pingCount + " " + host;
    ...
    public static int checkNetwork(String host) {
        ...
        if (TextUtils.isEmpty(host)) host = "119.29.29.29";
        long time = new NetPingManager(Utils.getApp().getApplicationContext(),
                host).getDelay2();
        Log.d(TAG, "checkNetwork time " + time);
        if (time == -1) return 2;
        return (int) time;
    }
