...
public class NetPingManager {
    ...
    private Handler mHandleMessage = null;
    ...
    public NetPingManager(Context context,String domain) {
        this.mDomain = domain;
     ...
     * 开始诊断网络
     */
    private long startNetDiagnosis() {
     ...
     * 使用java执行connected
     */
    private long execUseJava() {
        if (mAddress != null && mAddressIpList != null) {
            int len = mAddress.length;
            if (len > 1) {
               return execIP(mAddress[0], mAddressIpList.get(0));
            }
        }
        return -1;
    }
     ...
     */
    private long execIP(InetAddress inetAddress, String ip) {
        long isConnected = 0;
        ...
        if (inetAddress != null && ip != null) {
            ...
            if (flag == -1) {
                ...
                if (count > 0) {
                    isConnected = time / count;
                    if (mIOnNetPingListener != null){
                        mIOnNetPingListener.ontDelay(isConnected);
                    }
                }
            }
        } else {
